/******************************************************************************
 * Bundle:  t006_t_ae_summ
 * Source:  m5/datasets/bv-car20-p1/analysis/adam/programs/reporting/t_ae_summ.sas
 *
 * Table 14.3.1 - Overview of Treatment-Emergent Adverse Events. The table body
 * below is the repo's t_ae_summ.sas unchanged: subject-level TEAE/Grade3-4/
 * serious/CRS/ICANS flags via a PROC SQL left join + max(case), PROC TRANSPOSE,
 * shell merge, and a PROC REPORT with spanned dose-level columns. The ADAM
 * library (adsl, adae) is supplied as a small in-memory mock by the bundle
 * autoexec so the whole table runs in isolation.
 ******************************************************************************/
