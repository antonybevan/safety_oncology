 ...
 * Purpose:      Git Push for SAS OnDemand for Academics
...
%macro git_pull_safe;
    ...
    data _null_;
        ...
        if rc = 0 then do;
            ...
            put "WARNING: Pull conflict detected (RC=" rc ").";
...
%macro git_commit_push;
    %if &PULL_STATUS = CONFLICT %then %do;
        %put ERROR: Cannot push due to unresolved conflicts.;
        %put ERROR: Run GIT_RECOVERY.sas first to reset local state.;
        %return;
    %end;
    
    %if %length(&my_pat) = 0 %then %do;
        %put ERROR: GitHub PAT not configured!;
        %put ERROR: Edit this file and set my_pat = your_token;
        %return;
    %end;
