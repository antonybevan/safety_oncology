...
/* ============================================================================
   AESI ONSET AND DURATION SUMMARY (SAP Table 12: 3.3, Section 8.2.2)
   QC Level: 3 (Primary endpoint related)
   
   Per SAP Section 8.2.2:
...
data aesi_detail;
    ...
    if not missing(ASTDT) and not missing(CARTDT) then
        ONSET_DAYS = ASTDT - CARTDT + (ASTDT >= CARTDT);
    else if ASTDY ne . then ONSET_DAYS = ASTDY;
    else if not missing(ASTDT) and not missing(TRTSDT) then
        ONSET_DAYS = ASTDT - TRTSDT + (ASTDT >= TRTSDT);
    else ONSET_DAYS = .;
    
    /* Calculate duration per SAP Section 8.2.2 */
    if not missing(AENDT) and not missing(ASTDT) then 
        ...
        _cutoff = input("&DATA_CUTOFF", yymmdd10.);
        if missing(_cutoff) then _cutoff = ASTDT;
        DURATION_DAYS = _cutoff - ASTDT + 1;
        ONGOING_FL = 'Y';
    end;
...
proc report data=aesi_report nowd split='*';
    ...
    footnote4 "AESI: CRS, ICANS, GvHD, Infections per SAP Section 8.2.2.";
run;
